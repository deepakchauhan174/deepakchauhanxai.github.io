// aibhai-embed.js  (single-file embed)
// Requires server endpoints:
//  GET  check_user.php?device_id=...    -> returns JSON { exists: true/false, name: "..." }
//  POST save_user.php  (JSON)           -> accepts { device_id, name, mobile, email, age } and returns success indicator
(function(){
// ========== CONFIG ==========
...
style.textContent =   /* AI Bhai embed single-file styles */   #aibhai-root{ position:fixed; bottom:18px; left:18px; z-index:2147483647; font-family: "Noto Sans", Roboto, sans-serif; }   #aibhai-avatar{ width:78px; height:78px; border-radius:50%; border:3px solid rgba(255,215,0,0.95); box-shadow:0 10px 36px rgba(0,0,0,0.45), 0 0 18px rgba(255,215,0,0.08); cursor:pointer; background-size:cover; background-position:center; display:flex; align-items:center; justify-content:center; overflow:hidden; }   #aibhai-chat{ width:360px; max-width:calc(100vw - 40px); height:480px; background:linear-gradient(180deg, rgba(10,10,10,0.98), rgba(20,20,20,0.96)); border-radius:14px; border:1.5px solid rgba(255,215,0,0.85); box-shadow:0 20px 50px rgba(0,0,0,0.55); overflow:hidden; display:flex; flex-direction:column; transform:translateY(28px) scale(0.995); opacity:0; transition:all .42s cubic-bezier(.2,.9,.2,1); }   #aibhai-chat.open{ transform:none; opacity:1; }   #aibhai-header{ padding:12px 14px; background:linear-gradient(90deg, rgba(255,215,0,0.98), rgba(255,185,50,0.98)); color:#111; font-weight:700; display:flex; align-items:center; justify-content:space-between; }   #aibhai-title{ display:flex; align-items:center; gap:10px; font-size:15px; }   #aibhai-messages{ padding:12px; flex:1; overflow:auto; display:flex; flex-direction:column; gap:10px; color:#fff; }   .aibhai-msg{ max-width:86%; padding:10px 12px; border-radius:10px; background:linear-gradient(180deg, rgba(0,0,0,0.55), rgba(0,0,0,0.42)); box-shadow:0 6px 16px rgba(0,0,0,0.35); color:#fff; white-space:pre-wrap; line-height:1.4; }   .aibhai-msg.bot{ align-self:flex-start; }   .aibhai-typing{ width:56px; height:14px; display:flex; gap:6px; align-items:center; }   .aibhai-typing span{ display:block; width:8px; height:8px; background:#fff; border-radius:50%; opacity:0.18; animation:ablink 1s infinite; }   .aibhai-typing span:nth-child(2){ animation-delay:0.12s; } .aibhai-typing span:nth-child(3){ animation-delay:0.24s; }   @keyframes ablink{ 0%{transform:translateY(0);opacity:0.18} 50%{transform:translateY(-5px);opacity:1} 100%{transform:translateY(0);opacity:0.18} }   .aibhai-btn{ display:inline-block; padding:9px 14px; border-radius:10px; font-weight:700; border:none; cursor:pointer; background:linear-gradient(90deg, rgba(255,215,0,0.98), rgba(255,190,40,0.98)); color:#111; box-shadow:0 10px 26px rgba(0,0,0,0.28); }   .aibhai-form{ background:linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.02)); padding:12px; border-radius:10px; }   .aibhai-form label{ color: #ffd071; font-weight:700; display:block; margin-top:8px; margin-bottom:6px; font-size:13px; }   .aibhai-form input{ width:100%; padding:9px; border-radius:8px; border:none; background:rgba(0,0,0,0.45); color:#fff; }   .aibhai-form .submit{ margin-top:10px; width:100%; padding:10px; border-radius:8px; cursor:pointer; border:none; font-weight:800; background:linear-gradient(90deg, rgba(255,215,0,0.98), rgba(255,190,40,0.98)); }   .aibhai-close{ background:transparent;border:none;font-size:20px;cursor:pointer; color:#111; }   @media (max-width:420px){ #aibhai-chat{ right:10px; left:10px; width:auto; } #aibhai-avatar{ right:14px; bottom:14px; } }  ;
...
chat.innerHTML =   <div id="aibhai-header">   <div id="aibhai-title">🤴 AI Bhai</div>   <div style="display:flex;align-items:center;gap:8px;">   <button class="aibhai-close" aria-label="Close">×</button>   </div>   </div>   <div id="aibhai-messages" aria-live="polite"></div>  ;
...
formWrap.innerHTML =   <label>नाम</label>   <input id="aib-name" placeholder="आपका नाम" />   <label>मोबाइल</label>   <input id="aib-mobile" placeholder="मोबाइल नंबर" />   <label>ईमेल</label>   <input id="aib-email" placeholder="Gmail" />   <label>उम्र</label>   <input id="aib-age" type="number" placeholder="उम्र" />   <button class="submit">सबमिट करें</button>  ;
